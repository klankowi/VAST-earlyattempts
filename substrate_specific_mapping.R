...
for(i in 1:4){
  ...
  for(j in 1:length(Year.list)
    ...
    # Plot
    hard <- ggplot(ecodata::coast)+
     geom_sf(data=abgrid, aes(fill=y, col=y)) +
    ...
    hard$labels$fill <- "Abund."
    hard$labels$colour <- "Abund."
    hard
    
    mix <- ggplot(ecodata::coast)+
      geom_sf(data=abgrid, aes(fill=y, col=y)) +
      #geom_sf(data=closed.areas, fill=NA, col='black')+
      ...
      theme(#legend.position = c(0.90, 0.20),
            plot.margin = margin(t=0.25, b=0.25, l=0, r=0, 'cm'))
    mix$labels$fill <- "Abund."
    mix$labels$colour <- "Abund."
    mix
    
    # # Save
    ggsave(hard,
          filename =
            paste0("C:/Users/klankowicz/Desktop/hardsub.png"),
          device="png")
    
    ggsave(mix,
           filename =
             paste0("C:/Users/klankowicz/Desktop/mixsub.png"),
           device="png")
...
newfish$Group <- factor(newfish$Group, levels=c('Under 40cm',
                                                '40-70 cm',
                                                'Over 70 cm'))
load(here('data/RData_Storage/substrate_specific_indices.RData'))
