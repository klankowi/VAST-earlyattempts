...
#### Make settings ####
user_region <- readRDS(here('data/RData_Storage/user_region_withwind.rds'))
#user_region <- user_region[with(user_region, order(Lon, Lat)),]
row.names(user_region) <- NULL
head(user_region)
table(user_region$STRATA)
strata_use <- data.frame('STRATA' = c("All", "EGOM", "GBK", 'SNE', 'WGOM', 'WIND'))
...
setwd(here("VAST_runs/wind_areas2"))

#### Run model ####
# Subsample
survs <- survs[survs$Year > 75,]
scaled.covars <- scaled.covars[scaled.covars$Year > 75,]
...
save.image('wind_area_stratas.RData')
