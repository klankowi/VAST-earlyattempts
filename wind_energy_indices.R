...
closed.areas <- st_transform(closed.areas, crs = projargs)

wind.areas <- st_read(here('data/GIS/BOEM_GoM_lease.shp'))
wind.areas <- st_transform(wind.areas, crs = projargs)
wind.areas <- dplyr::select(wind.areas,
...
wind.cells <- st_intersects(wind.areas, grid)

grid$Cell <- seq(1, nrow(grid))

grid$Wind[grid$Cell %in% wind.cells[[1]]] <- 1

st_area(st_union(grid[grid$Wind == 1,]))

ggplot(coast) + 
  geom_sf(data=grid, col='black', aes(fill=Inc)) +
  geom_sf(data=wind.areas, col='red', fill=NA) +
  geom_sf(fill='lightgray', col=NA) +
...
Points_orig = sp::SpatialPointsDataFrame(coords = loc_g, 
                                         data = data.frame(wind = rep(NA, nrow(loc_g))),
                                         proj4string = CRS_orig)
...
wind.pts <- Points_sf[st_intersects(Points_sf, wind.areas)]
