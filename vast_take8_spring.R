...
#### Make settings ####
setwd(here("VAST_runs/StrataDens_Spring"))
...
#### Run model ####
survs.spring <- subset(survs, SEASON == 'SPRING')
covars.spring <- subset(scaled.covars, SEASON == 'SPRING')

rm(list=setdiff(ls(), c("covars.spring", "strata_use", "survs.spring",
                        "vast_extrap_info", "settings")))
gc()
...
fit = fit_model( 
  ...
  # Call survey data info
    Lat_i = survs.spring[,'Lat'], 
    Lon_i = survs.spring[,'Lon'], 
    t_i = survs.spring[,'Year'], 
    b_i = survs.spring[,'COD_N'], 
    a_i = survs.spring[,'swept'], 
    v_i = survs.spring[,'vessel'],
...
#save.image('strata_covs_spring.RData')
