...
outliers <- boxplot.stats(D$D)$out
D$D[D$D %in% outliers] <- NA
...
for(i in 1:length(D.list)){
  ...
  for(j in 1:length(Year.list)
    ...
    p$labels$fill <- "log(Abundance)"
    p$labels$colour <- "log(Abundance)"
    
    # Save
    ggsave(p, 
           filename = 
             paste0("C:/Users/klankowicz/Desktop/VAST_examples/Mapping/logabund_closed/",
                    names(D.list)[i], "/",
...
files  <- list.files(paste0("C:/Users/klankowicz/Desktop/VAST_examples/",
                            "Mapping/logabund_closed/Ages 5+"))
setwd("C:/Users/klankowicz/Desktop/VAST_examples/Mapping/logabund_closed/")
files <- paste0(getwd(), "/Ages 5+/", files)

# Set GIF save location
setwd("C:/Users/klankowicz/Desktop/VAST_examples/Mapping/logabund_closed/GIFs/")

# Convert PNGs to GIF
library(gifski)
gifski(files, "Ages5+_logabund_closed.gif", loop = FALSE, delay = 0.25)
