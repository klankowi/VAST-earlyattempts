...
# Surveys
surveys <- read.csv(here("Data/Survey_Data/Survey_Data.csv"))
surveys <- subset(surveys, !is.na(LAT))
surveys <- subset(surveys, YEAR > 1981 & YEAR < 2022)
...
saveRDS(grid, file=here('Data/Density_Covariates/Sediment/sediment_interp_grid_hires.RDS'))
#grid <- readRDS(here('Data/Density_Covariates/Sediment/sediment_interp_grid_hires.RDS'))
...
wss
# 9 groups gives 96% stability

set.seed(123)
c <- kmeans(grid_noNA, centers = 9, nstart= 99, iter.max = 99)
grid$cluster[!is.na(grid$cobble)] <- c$cluster
cent.vals <- c$centers
cent.vals <- as.data.frame(cent.vals)
cent.vals$num <- seq(1:9)
...
# Outcome
# Groups 1 and 8 are "harder" (high cobble)
# Groups 4, 5, 7, 8, 9 are "mixed" (high gravel or some cobble)
# Groups 2,3,6 are smooth (no cobble and low gravel content)

grid$outcome <- NA
grid$outcome[grid$clust %in% c(1, 8)] <- 'Hard'
grid$outcome[grid$clust %in% c(4, 5, 7, 8, 9)] <- 'Mix'
grid$outcome[grid$clust %in% c(2, 3, 6)] <- 'Soft'

sed.dist <- ggplot(coast) +
  geom_sf(data=grid[!is.na(grid$outcome),], 
          col=alpha('black', 0.1),
          aes(fill=as.factor(outcome))) +
  scale_fill_viridis_d(na.value = NA, option='viridis', name="Sediment\ntype") +
  geom_sf(fill='gray') +
  coord_sf(xlim=c(-125*1000, 780*1000),
           ylim=c(4050*1000,5000*1000),
           crs="EPSG:32619")
...
ggsave(sed.dist, 
       filename = 
         paste0(here(),
                '/Plot_output/sediment_distribution.png'),
       device="png",
       width = 8.5, 
       #height = 11, 
       units='in'
...
rm(c, cent.vals, coast, data.cats, data.f, data.list, grid_noNA, hospital_names,
   in.hard, in.mix, pb, seds, surveys, temp, test, use.cell, use.seds, 
   cat.labs, i, substrates, wss, hospital_labeller)

save.image(here('Data/Density_Covariates/Sediment/sediment_grids.RData'))
