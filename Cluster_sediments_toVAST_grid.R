...
for(i in 1:length(substrates)){
  data.use <- as.data.frame(data.list[[i]])
  names(data.use) <- 'sub'
  data_noNA <- data.use[complete.cases(data.use),]
  set.seed(123)
  c <- kmeans(data_noNA, centers = 2, nstart= 10, iter.max = 20)
  ...
  grid$c2[is.na(data.use$sub)==FALSE] <- c$cluster
  data.cats[,paste0(substrates[i])][is.na(data.use$sub)==FALSE] <- c$cluster
...
# Groups 7,8,4,10 are "mixed" (high gravel)
# Groups 1,3,6,9 are smooth (low cobble and gravel content)
...
ggplot(coast) +
  geom_sf(data=test, pch=19, cex=0.5) +
  geom_sf(data=cob, fill=NA, col='black') +
  geom_sf() +
  geom_sf(data=closed.areas, col='red', fill=NA) +
  coord_sf(xlim=c(100*1000, 780*1000),
           ylim=c(4500*1000,4960*1000),
           crs="EPSG:32619")+
  xlab('Longitude') +
  ylab('Latitude') +
